...
function crearTablaConsulta(porPtj) {
    ...
    if (porPtj) {
        array = listapptj;
        url_rep = bdir + 'admin/reporte_consulta/puntaje';
    } else {
        array = listadatos;
        url_rep = bdir + 'admin/reporte_consulta';
    }
...
function loadClick() {
    $('#tabla-consul input').off().on('click', function () {
        var id = parseInt($(this).attr('id').split('-')[2], 10);
        //console.log(id);
        var form = $('<form></form>');
        form.attr('method', 'post');
        form.attr('action', bdir + 'admin/editar_usuario');
        var field = $('<input></input>');
        field.attr('type', 'hidden');
        field.attr('name', 'q-id');
        field.attr('value', id);
        form.append(field);
...
$(document).ready(function () {
    ...
    $('#q-submit').click(function () {
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: 'json',
            url: bdir + 'ajaxadm/consulta_usuarios'
        }).done(function (data, textStatus, jqXHR) {
