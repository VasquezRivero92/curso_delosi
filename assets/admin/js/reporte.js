...
function loadSelectCCosto() {
    ...
    xhrCC = $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        url: bdir + 'ajaxadm/get_sede_emps'
...
$(document).ready(function () {
    ...
    $('#r-submit').click(function () {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            url: bdir + 'ajaxadm/reporte_tabla'
        }).done(function (data, textStatus, jqXHR) {
            $('#r-submit').removeClass('disable');
            $('#load-reporte').hide();
            if (data.length) {
                var tabla = '<a href="' + bdir + 'admin/reporte_descarga" id="rep-descarga" target="_blank">Descargar reporte</a>';
