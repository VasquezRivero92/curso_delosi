...
/*******************************************************************************/
function Lab_PowerUp(x, y, ancho, alto, direccion) {
...
/***
 function Lab_MiniMap(id) {
 Lab_Objeto.call(this, 0, 0, 0, 100);
 this.escala = 0;
 this.ctx;
 this.update = function () {
 this.ctx = $("#miniMap" + id).get(0).getContext('2d');
 this.width = 100 * $J[$JAct].width / $J[$JAct].height;
 this.escala = this.width / $J[$JAct].width;
 $("#miniMap" + id).css("width", this.width + "px").attr({"width": this.width,"height": this.height});
 };
 this.initDraw = function () {
 this.ctx.clearRect(0, 0, this.width, this.height);
 };
 this.draw = function (x, y, width, height, color, alpha) {
 if (alpha === undefined) { alpha = 1; }
 var E_x = this.escala * x;
 var E_y = this.escala * y;
 var E_w = this.escala * width;
 var E_h = this.escala * height;
 this.ctx.fillStyle = color;
 this.ctx.globalAlpha = alpha;
 this.ctx.fillRect(E_x, E_y, E_w, E_h);
 this.ctx.fill();
