...
var $stopTimeCrono = 0;
var $delayTime = 10; // normal valor 20
...
var $PowerPR = [];
var $ActPwrUp = 0;
var $Preguntas = [null, 1];
var $slA = [null,
    'Una colilla de cigarro encendio material combustible en una papelera, ¿Qué tipo de fuego es y que extintor deberias usar?',
    'Un equipo se sobrecaliento cerca de material combustible y se encendio un fuego ¿qué tipo de fuego es y que extintor deberias usar?',
    'El cable pelado de un equipo a generado un corto circuito y encendido un corto circuito cerca de material combistible produciendo un incendio ¿qué tipo de fuego es y que extintor deberias usar?',
    'Una fuga de gas cerca  a una cocina encendida ha prendido una llama cerca a material inflamable. ¿Qué tipo de fuego es y que extintor deberías usar?',
    'Restos de grasa acumulada por falta de limpieza han servido de base para la inciación de un fuego ¿qué tipo de fuego es y que extintor deberías usar? '
];
var $slB = [null,'aaa'];
var $slC = [null,'aaa'];
...
function initBotones() {
    ...
    $('#btnReintento').click(function () {
        ...
        $J[$JAct].CTiempo = $J[$JAct].CTiempo + 1;
        isPaused = false;
        $("#icon_" + ($J[1].contAciertos + 1)).removeClass('anim error check').addClass("anim error");
    });

    $('#btnListo').click(function () {
        playSound(window.audioCatch);
        $("#icon_" +  $J[1].contAciertos).removeClass('anim check error').addClass("anim check");
        $ActPwrUp = 0;
        $('#pregWindow').stop().fadeOut(400);
        $('.caratula').stop().delay(500).fadeOut(10);
...
function muestraPregunta() {
    ...
    if ($ActPwrUp == 1 || $ActPwrUp == 2 || $ActPwrUp == 3 || $ActPwrUp == 4 || $ActPwrUp == 5) {
        $("#pregMain").show();
        stage = $ActPwrUp-1;
        game = new Phaser.Game(config);
    }else{
        $("#pregBien_opcinal").show();
    }
}
$.fn.extend({
