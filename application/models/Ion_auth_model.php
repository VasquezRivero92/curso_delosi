...
class Ion_auth_model extends CI_Model {
    ...
    public function hash_password_tienda($id, $password, $use_sha1_override = FALSE) {
        if (empty($id) || empty($password)) {
            return FALSE;
        }
        $this->trigger_events('extra_where');
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, avatar, id_nivel, id_grupo, active, last_login')
                ->where($this->identity_column, $identity)
                ->or_where('email', $identity)//identity_column es el DNI y adicional checkeo el email como secundario
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
    ...
    public function loginTienda($username, $password, $remember = FALSE) {
        $this->trigger_events('pre_login');
        if (empty($username) || empty($password)) {
            $this->set_error('login_unsuccessful');
            return FALSE;
        }
        $query = $this->db->select('id,marca,formato,usuario,usuario2,nombre,password,active,id_admin')
                ->where('usuario', $username)
                //->or_where('usuario2', $username)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get('tienda');
        ...
        if ($query->num_rows() === 1) {
            $user = $query->row();
            $password = $this->hash_password_tienda($user->id, $password);
            if ($password === TRUE) {
                if ($user->active == 0) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_not_active');
    ...
    public function nivel_usuario($id = FALSE) {
        $this->trigger_events('nivel_usuario');
        $id || $id = $this->session->userdata('user_id');
        return $this->db->select('id_nivel')
                        ->where('id', $id)
                        ->limit(1)
                        ->get($this->tables['users'])
                        ->row()->id_nivel;
    ...
    public function set_session($user) {
        ...
        $queryFIP = $this->db->select_min('fingpla')
                        ->where('id_user', $user->id)
                        ->get('users_empresa')->row();
        if ($queryFIP) {
            $fingpla = $queryFIP->fingpla;
        } else {
            $fingpla = time();
        }
        if ($user->id_grupo == 1) {
            $grupo = 'restaurante';
        } elseif ($user->id_grupo == 2) {
            $grupo = 'planta';
    ...
    public function set_session_tienda($user) {
        $this->trigger_events('pre_set_session');
        $session_data = array(
    ...
    public function login_remembered_user() {
        ...
        // get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', email, id, password, id_nivel, id_grupo, active, last_login')
    ...
    public function get_id_vers() {
        return $this->db->select('id')
                        ->where('nombre', $this->config->item('site_version', 'ion_auth'))
                        ->get('version')->row()->id;
    }
